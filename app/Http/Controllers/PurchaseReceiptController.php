...
class PurchaseReceiptController extends Controller
{
   ...
   /**
    ...
    public function store(Request $request)
    {
        ...
        $last = PurchaseReceipt::whereYear('date', date($date->format('Y')))->whereMonth('date',date($date->format('m')))->orderBy('serial', 'DESC')->first();
        if($last == null){
            $last = 0;
        }else{
            $last = $last->serial + 1;
    ...
    */
    ...
    public function update(Request $request,$id)
    {
    	...
    	$date = Carbon::createFromFormat('d-m-y\TH:i',$request->date)->format('ym');
    	$serial_no = 'PR-'.$date.$serial;
    	
    	
    	$request->merge(['serial_no' => $serial_no]);
    	$request->validate([
    		'serial_no' => 'required|unique:purchase_receipts,serial_no,'.$module->id,
    		'date' => 'required',
    	]);
