...
@section('title', 'Edit Product Sale Order')
@section('css')
    <style>
        .normal-btn {
            background: white;
...
@section('content')
    <script>
        ...
        var existing_items = @json($product_sale_order->items);
    </script>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18">Edit Product Sale Order</h4>
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="/">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('product_sale_orders.index') }}">Product Sale Orders</a></li>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="bg-white container-fluid">
                            <div class="row">
                                <div class="col-6">
                                    <h4 class="card-title">Edit: {{ $product_sale_order->serial_no }}</h4>
                                </div>
                ...
                <form method="post" action="{{ route('product_sale_orders.update', $product_sale_order->id) }}">
                    @csrf
                    @method('PUT')
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <input required name="date" type="date" 
                                                    value="{{ old('date', $product_sale_order->date->format('Y-m-d')) }}"
                                                    class="form-control" />
                                                @if ($errors->has('date'))
                                                ...
                                                <select name="customer_id" class="form-control" required>
                                                    ...
                                                    @foreach ($customers as $customer)
                                                        <option data-address="{{ $customer->address }}"
                                                            value="{{ $customer->id }}"
                                                            {{ $customer->id == $product_sale_order->customer_id ? 'selected' : '' }}>
                                                            {{ $customer->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-5">
                                            <div class="form-group">
                                                ...
                                                <input readonly type="text" class="customer_address form-control" 
                                                    value="{{ $product_sale_order->customer->address }}" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select name="payment_method" class="form-control" required>
                                                    <option value="cash" {{ $product_sale_order->payment_method == 'cash' ? 'selected' : '' }}>Cash</option>
                                                    <option value="loan" {{ $product_sale_order->payment_method == 'loan' ? 'selected' : '' }}>Loan</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="descr" class="form-control" 
                                                    value="{{ old('descr', $product_sale_order->descr) }}" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            if (existing_items && existing_items.length > 0) {
                existing_items.forEach(function(item) {
                    ...
                    $('.line-items').append(`
                        <div class="row py-1"> 
                            <div class="col-md-4">
                               <input class="line_item_id" type="hidden" name="items[${index}][id]" value="${item.product_id}" />
                               <input readonly name="items[${index}][name]" class="form-control" value="${item.product.name} ${item.product.unit}" />
                            </div>
                            <div class="col-md-2">
                                <input min="1" value="${item.qty}" step=".01" required name="items[${index}][qty]" type="number" class="form-control" />
                            </div>
                            <div class="col-md-2">
                                <input min="1" value="${item.rate}" step=".01" required name="items[${index}][rate]" type="number" class="form-control" />
                            </div>
                            <div class="col-md-3">
                                <input required name="items[${index}][dispatched_through]" type="text" value="${item.dispatched_through}" class="form-control" required/>
                            </div>
