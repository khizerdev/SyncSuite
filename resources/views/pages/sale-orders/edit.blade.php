...
@section('content')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-secondary">
                        ...
                        <div class="card-body">
                            <form action="{{ route('sale-orders.update', $saleOrder->id) }}" method="POST" id="editForm">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="customer_id" id="customer_id" class="form-control" required>
                    @foreach ($customers as $customer)
                        <option value="{{ $customer->id }}" {{ $saleOrder->customer_id == $customer->id ? 'selected' : '' }}>
                            {{ $customer->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="order_status" id="order_status" class="form-control" required>
                    <option value="open" {{ $saleOrder->order_status == 'open' ? 'selected' : '' }}>Open</option>
                    <option value="hold" {{ $saleOrder->order_status == 'hold' ? 'selected' : '' }}>Hold</option>
                    <option value="cleared" {{ $saleOrder->order_status == 'cleared' ? 'selected' : '' }}>Cleared</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="form-section card mb-3" style="zoom:0.9" data-item-id="{{ $item->id }}">
        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
            ...
            <div class="col-md-1">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control lace-qty" name="lace_qty[]"
                        min="0" step="1" value="{{ $item->lace_qty }}" required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
            ...
            <div class="col-md-1">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control calculate_stitch"
                        name="calculate_stitch[]" value="{{ $item->calculate_stitch }}" readonly required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
            ...
            <div class="col-md-1 d-flex align-items-end">
                <div class="form-group w-100">
                    <button type="button" class="btn btn-success btn-block add-section"><i class="fas fa-plus"></i></button>
                    <button type="button" class="btn btn-danger btn-block remove-section mt-1"><i class="fas fa-minus"></i></button>
                </div>
            </div>
        </div>
        <!-- Hidden field to track existing items -->
        <input type="hidden" name="item_ids[]" value="{{ $item->id }}">
    </div>
    @endforeach

    <!-- If no existing items, show one empty section -->
    ...
    <div class="form-section card mb-3" style="zoom:0.9">
        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <select class="form-control design-name" name="design_name[]" required>
                        <option value="">Select Design</option>
                        @foreach (\App\Models\FabricMeasurement::all() as $design)
                            <option value="{{ $design->id }}" data-stitch="{{ $design->design_stitch }}">
                                {{ $design->design_code }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-1">
                <div class="form-group">
                    <label>Qty</label>
                    <input type="number" class="form-control qty" name="qty[]" min="0" step="1" required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Lace Qty</label>
                    <input type="number" class="form-control lace-qty" name="lace_qty[]" min="0" step="1" required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Rate</label>
                    <input type="number" class="form-control rate" name="rate[]" readonly required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Stitch</label>
                    <input type="text" class="form-control stitch" name="stitch[]" readonly required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Stitch Rate</label>
                    <input type="number" class="form-control stitch-rate" name="stitch_rate[]" 
                        value="{{ \App\Models\Setting::find(2)->value }}">
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Calc Stitch</label>
                    <input type="text" class="form-control calculate_stitch" name="calculate_stitch[]" readonly required>
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>L/F</label>
                    <input type="number" class="form-control length-factor" name="length_factor[]" 
                        value="{{ \App\Models\Setting::find(1)->value }}">
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
                    <label>Amount</label>
                    <input type="text" class="form-control amount" name="amount[]" readonly required>
                </div>
            </div>
            <div class="col-md-1 d-flex align-items-end">
                <div class="form-group w-100">
    ...
    <a href="{{ route('sale-orders.index') }}" class="btn btn-secondary">Cancel</a>
</form>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    function addNewSection(afterSection) {
        ...
        newSection.find('.stitch-rate').val({{ \App\Models\Setting::find(2)->value }});
        newSection.find('.length-factor').val({{ \App\Models\Setting::find(1)->value }});
        
        // Clear item ID for new sections
        newSection.find('input[name="item_ids[]"]').val('');
        newSection.removeAttr('data-item-id');
